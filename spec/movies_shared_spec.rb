...
RSpec.shared_examples "a movie with limited period and certain price" do |year_range, expected_period, expected_price|
  let(:record) {CSV.read('..\src\movies.txt', col_sep: "|").map.find{|a| year_range.include?(a[2].to_i)}}
  let(:bad_record) {CSV.read('..\src\movies.txt', col_sep: "|").map.find{|a| !(year_range.include?(a[2].to_i))}}
...
def make_movie(movie_class, host = nil)
  record = CSV.read('..\src\movies.txt', col_sep: "|").map.find{ |a| movie_class::PERIOD.include?(a[2].to_i) }
